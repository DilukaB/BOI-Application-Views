...
const generatePdf = (formData) => {
  ...
  if (formData.fuelConsumptions?.length) {
    ...
    y = addWrappedText("Note: Specify other fuel types if applicable and include consumption estimates.", margin, y);
    doc.setTextColor(0);
    y += 10;
  }
  ...
  if (formData.waterConsumptions?.length) {
    ...
    doc.setTextColor(100);
    y = addWrappedText("Note: Please include all sources and anticipated changes based on capacity.", margin, y);
    doc.setTextColor(0);
    y += 10;
  }
